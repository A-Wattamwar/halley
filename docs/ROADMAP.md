...
| 2026-05-29 | 0.7 | **Phase 5 complete (Weeks 9–10).** Full hero loop shipped and validated: `halley record` (bit-fidelity Python shim via sitecustomize, D22 canonical hash parity proven Python↔Rust) → `halley ci` (pure-mode $0 replay, ordinal cursor, JUnit XML, all invariant types) → `halley diff` (human-readable drift delta) → `halley bisect` (binary search, 3× retry, repo restore). Hybrid mode + tool-effect-safe replay + bisect.run BullMQ job + dashboard BisectPanel. GitHub Action wrapping `halley ci`. Hero demo: `~/halley-hero-demo/` with authentic shim-recorded fixture (5 spans, $0.000054 original spend) — full ci→bisect loop in 6.6 s, $0. Fixture format v1 LOCKED (D52, D53). D53 mid-phase pivot (dual-mode shim, provisional→locked). Debt: D22 triplication → extract `halley-canonical` crate (Phase 6 P0). See phase-5-overview.md retro for full debt inventory. | The hero loop is the product. Phase 5 delivers every layer: ingest → infer → record → replay → diff → bisect → CI. |
| 2026-06-01 | 0.8 | **Phase 6 complete (Weeks 11–12) — launched.** Week 11 closed the dashboard half of the loop: `halley-canonical` crate extracted (D22 single source of truth, parity-guarded in CI), per-fixture execution context (no hardcoded paths), dashboard Run CI / runner-status pill / copy-command UI, runner architecture **D54** (host runner executes CI + bisect; terminal commands always shown), `HALLEY_READ_ONLY` demo guard (**D55**), ingester root-context Docker build fix. Week 12 shipped launch assets: README hero GIF + Week 11 screenshots + green-CI badges, docs index + cross-linking + honesty pass, static landing page, video script + blog draft. Repo history scrubbed of internal planning docs and made public. **North-star status:** public repo + polished README ✅; `docker compose up` ✅; three example apps ✅; `halley` CLI + GitHub Action ✅; 4,792 spans/sec load test ✅; four-dialect property tests ✅; hero demo (prompt change → red CI → bisect names the commit) ✅; hero GIF ✅. **Honestly deferred:** TS shim (placeholder only), semantic invariant runner (stub, off), GitHub App fixture push, promote-path auto-population of `target_repo_path`; live hosted demo + 3-min video upload + blog publish are post-launch [AYUSH] steps. | The loop is the product; Phase 6 made it visible, honest, and public. |
